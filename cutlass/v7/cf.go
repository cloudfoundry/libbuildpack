...
func New(fixture string) *App {
	return &App{
		Name:         ensureValidAppName(filepath.Base(fixture) + "-" + cutlass.RandStringRunes(20)),
		Path:         fixture,
		Stack:        os.Getenv("CF_STACK"),
		Buildpacks:   []string{},
		Memory:       cutlass.DefaultMemory,
		Disk:         cutlass.DefaultDisk,
		StartCommand: "",
		appGUID:      "",
		env:          map[string]string{},
		logCmd:       nil,
...
func ApiVersion() (string, error) {
	...
	cmd.Stderr = cutlass.DefaultStdoutStderr
...
func (a *App) PushNoStart() error {
	...
	if a.logCmd == nil {
		...
		a.logCmd.Stderr = cutlass.DefaultStdoutStderr
...
func (a *App) GetBody(path string) (string, error) {
	body, headers, err := a.Get(path, map[string]string{})
