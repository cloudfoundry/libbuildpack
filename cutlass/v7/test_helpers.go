...
func PackageUniquelyVersionedBuildpack(stack string, stackAssociationSupported bool) (VersionedBuildpackPackage, error) {
	...
	return PackageUniquelyVersionedBuildpackExtra(strings.Replace(manifest.Language, "-", "_", -1), buildpackVersion, stack, cutlass.Cached, stackAssociationSupported)
}
...
func PackageShimmedBuildpack(bpDir, stack string) (VersionedBuildpackPackage, error) {
	...
	cutlass.DefaultLogger.Debug("package-shimmed-buildpack")
	...
	if bpFilePath == "" {
		...
		bpFilePath, err = archiver.Archive(bpDir, stack, timestamp, cutlass.Cached)
		if err != nil {
